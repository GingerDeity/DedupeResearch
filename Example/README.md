There are 3 memory dumps here, collected by running ./map_and_core.sh on a ./hello process. You can easily set up an environment to do this yourself, but we've included 3 memory dumps that will allow you to perfectly follow along and verify your code is working.

# Basics
First, unzip the hello1.tar.gz file, and run the command: `elf hello1.2179878 hello1_P.2179878` to remove all the metadata from the hello1 memory dump. You'll see that the code outputs all sections it writes and a final number showing the new file size. If you want to verify correct output, you can compare this output to the hello1_elf_output.txt attached. Now that we have a working memory dump, let's run deduplication on it!  
  
After compiling your own version of DedupeCheck.java, run `java DedupeCheck 4096 hello1_P.2179878`, you'll see the following information:
```
Files:          1
Dedupe Graph:   [0.00%]
Unique Windows: 67
...
You'll see that the zero-windows + duped data + total data = the file size. A low deduplication ratio is to be expected for this, but let's see what happens when we do it with 2 nearly identical files. Let's unzip hello2.tar.gz run the `elf` code on it (which I'll be calling hello2_P.2179948). Once that's done, let's try running `java DedupeCheck 4096 hello1_P.2179878 hello2_P.2179948`. You should see the following results:  
```
Files:          2
...
As expected, deduplication ratios are much higher! If we do this with all three hello world files: `java DedupeCheck 4096 hello1_P.2179878 hello2_P.2179948 hello3_P.2179967`
```
Files:          3
...
Now for a tutorial on quickly getting CSV files. Place the 3 parsed dumps into their own directory. I'm placing mine in a direcotry called 'test'. Go into your DedupeCheckFull.sh and DedupeCheckList.sh and make sure they point to the correct location of your deduplication code. After that, run `./DedupeCheckFull.sh 4096 test/` and you'll see that it's outputting matches for up to 2 files! To verify output, compare against the dedupe_full_output.txt file included. Let's use that text file and run `java ParseStatic dedupe_full_output.txt DR -f 3`. You should see the following output:
...
Bam! A quick CSV file containing all the deduplication ratios from comparing 2 files. You'll notice that the major diagonal shows the results for 1 file instead of 2. You shouldn't expect to find anything interesting from comparing 2 exact copies of a file. That will always result in ratios of at least 50%. Let's say we want a CSV file of hello2_P against all the other files, I've already attached text file of what that output would be, done by running `./DedupeCheckList.sh 4096 test/hello2_P.2179948 test/`. Simply run `java ParseStatic dedupe_list_output.txt DR -l 3` to get the following output:
```
...
# Extra Analysis
For this part, assuming your parsed files are still in `/test`, let's run the following command: `java DedupeCheck 4096 -v test/hello1_P.2179878 test/hello2_P.2179948 >> hello1vhello2_matches.txt`
...
This tells us there are 47 matches that are both 4kB in terms of alignment and length, and that the code detected 47 matches total. Recall when we did our hello1_P vs hello2_P deduplication from earlier, there was 192512 bytes of duplicate data. Each match should be 4kB long, so 192512/4096 = 47 matches total. This helps show us our modification didn't destroy any functionality! Let's say we were curious how many of these matches happened to be 8kB aligned, just run `java ParseMatches 8192 hello1vhello2_matches.txt`. You should see:
```
...
Now let's say we want to see the distances between matching blocks across hello1. We will need a 64B deduplication report of hello1, so run `java DedupeCheck 64 -v test/hello1_P.2179878 >> hello1_matches64.txt`. From there, just run `python3 blockanalyze.py hello1_matches64.txt` and see:
```
